...
const correct =
    guessWords[Math.floor(Math.random() * guessWords.length - 1)].toUpperCase();
let defaulBoard = [];
let defaultLetters = [];

"абвгдеёжзийклмнопрстуфхцчшщъыьэюя".split("").forEach((i) => {
    defaultLetters[i] = "";
});
                    ...
                    setBoard((prevBoard) => {
                        if (col < 5) {
                            if (props.letter !== "ENTER") {
                                ...
                                props.error("Слово должно состоять из 5 букв!");
                                setTimeout(() => {
                                    props.error("");
                                }, 1000);
                            }
                                ...
                                if (words.includes(word.toLowerCase())) {
                                    for (let i = 0; i < 5; i++) {
                                        ...
                                        if (row === 5) {
                                            setLost(true);
                                            setTimeout(() => {
                                                setMessage(`Загаданное слово было ${correct}`);
                                            }, 750);
                                        }
                                    ...
                                    props.error("Такого слова мы не знаем");
                                    setTimeout(() => {
                                        props.error("");
                                    }, 1000);
                                }
